...
pub fn handle_char_inp(editor_data: &mut util::EditorDataStruct, character: char) {
    let mut curfile = &mut editor_data.files[editor_data.current_file_index];
    let mut curline = &mut curfile.file_lines[curfile.cursor_y];
    curline.line_text.insert(curfile.cursor_x, character);
...
pub fn handle_remove(editor_data: &mut util::EditorDataStruct, isDel: bool) {
    let mut file_data = &mut editor_data.files[editor_data.current_file_index];
    let mut curline = &mut file_data.file_lines[file_data.cursor_y];

    if isDel {
        curline.line_text.remove(file_data.cursor_x);
    } else {
        ...
        if file_data.cursor_x == 0 && curline.line_length > editor_data.settings.line_endings {
            let cursline = file_data.file_lines[file_data.cursor_y].line_text.clone();
            file_data.file_lines.remove(file_data.cursor_y);
            file_data.cursor_y -= 1;
            file_data.file_lines[file_data.cursor_y].line_text.pop();
            file_data.file_lines[file_data.cursor_y].line_text.pop();
            file_data.file_lines[file_data.cursor_y].line_text.push_str(&cursline);
            curline = &mut file_data.file_lines[file_data.cursor_y];
            file_data.cursor_x = curline.line_length-(editor_data.settings.line_endings);
        ...
        curline.line_text.remove((file_data.cursor_x-1));
